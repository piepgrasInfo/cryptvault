...
| S4 | With biometrics enabled, the masterkey copy is decryptable only after a successful BIOMETRIC_STRONG (or device-credential on API 30+) authentication within the last 300 s; enrolment changes invalidate it and the app falls back to the password without error dialogs beyond one line | P2 |
| S5 | The recovery key resets the password without any other secret; a wrong word or bad CRC is rejected before anything is written | P2 |
| S6 | Every window (activity, dialog, popup, DocumentsProvider-triggered UI) is `FLAG_SECURE` | P2 |
| S7 | Clipboard content placed by the app is flagged sensitive and cleared after 60 s | P2 |
| S8 | The DocumentsProvider returns no roots while every vault is locked and stops serving reads the moment a vault locks (open descriptors fail, they are not kept alive) | P3 |
| S9 | Backups contain no cleartext and no cleartext names; the snapshot manifest is unreadable without the vault password | P4 |
| S10 | A backup run that is interrupted at any byte leaves the remote folder restorable from the previous commit point | P4 |
| S11 | Provider tokens are stored encrypted under a Keystore key; revoking access in the app deletes them and, where the API allows, revokes the token server-side | P4 |
